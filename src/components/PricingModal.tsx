...
export function PricingModal({ open, onClose }: PricingModalProps) {
  ...
  return (
    <div className="modal-overlay-fade fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4 py-6 backdrop-blur-md">
      <div className="modal-panel-pop relative w-full max-w-4xl overflow-hidden rounded-3xl border border-slate-700/80 bg-slate-900 p-6 shadow-2xl sm:p-8">
        {/* Close Button */}
        <button
          type="button"
          onClick={onClose}
          ...
          <div className="flex flex-col justify-between rounded-2xl border border-slate-800 bg-slate-950/60 p-6 transition hover:border-slate-700">
            <div>
              <div className="flex items-center justify-between">
                ...
                <span className="rounded-full bg-slate-800 px-3 py-1 text-xs text-slate-300">Free Forever</span>
              </div>
          ...
          <div className="relative flex flex-col justify-between rounded-2xl border-2 border-violet-500/80 bg-linear-to-b from-violet-950/30 to-slate-950/80 p-6 shadow-xl shadow-violet-950/20">
            ...
            <div>
              ...
              <ul className="mt-6 space-y-3 text-xs text-slate-200">
                ...
                <li className="flex items-center gap-2">
                  <Check className="h-4 w-4 text-violet-400" />
                  Custom Checklist Items (Tambah aturan sendiri)
                </li>
                <li className="flex items-center gap-2">
                  <Check className="h-4 w-4 text-violet-400" />
